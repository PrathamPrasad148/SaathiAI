...
class ChatStreamView(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.chat_log.tag_configure("user_hdr", foreground="#38bdf8", font=("Segoe UI", 10, "bold"))
        ...
        self.chat_log.tag_configure("asst_body", foreground="#e2e8f0", font=("Segoe UI", 10))
        self.chat_log.tag_configure("tool_action", foreground=COLOR_AMBER, font=FONT_MONO)
        self.chat_log.tag_configure("code_block", foreground="#38bdf8", background="#051329", font=FONT_MONO)
    ...
    def append_message(self, role: str, text: str):
        ...
        if role == "user":
            self.chat_log.insert("end", f"[{t_str}] DIRECTIVE ❯ ", "user_hdr")
            self.chat_log.insert("end", f"{text}\n\n", "user_body")
        elif role == "assistant":
            self.chat_log.insert("end", f"[{t_str}] SAATHI ❯ ", "asst_hdr")
            self.chat_log.insert("end", f"{text}\n\n", "asst_body")
        elif role == "action":
            self.chat_log.insert("end", f"⚡ [{t_str}] {text}\n", "tool_action")
        elif role == "system":
            self.chat_log.insert("end", f"🛡️ [{t_str}] {text}\n\n", "tool_action")
        self.chat_log.configure(state="disabled")
