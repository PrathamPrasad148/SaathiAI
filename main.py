...
class Saathi:
    ...
    def configure_theme(self) -> None:
        ...
        style.configure("TButton", background="#122a47", foreground="#dff7ff", borderwidth=0, padding=(10, 6))
        style.map("TButton", background=[("active", "#1d5c7a")])
        style.configure("TNotebook", background="#050914", borderwidth=0)
        style.configure("TNotebook.Tab", background="#0b1629", foreground="#9bb9d4", padding=(14, 8))
    ...
    def build_ui(self) -> None:
        ...
        ttk.Button(left, text="Grant session access", command=self.toggle_routine_permissions).pack(fill=tk.X, padx=14, pady=3)
        ...
        ttk.Button(right, text="Speak to Saathi", command=self.listen).pack(fill=tk.X, padx=14, pady=(10, 3))
        ...
        self.chat = scrolledtext.ScrolledText(chat_tab, state="disabled", wrap=tk.WORD, font=("Segoe UI", 11), bg="#08101e", fg="#d6efff", insertbackground="#6fe9ff", relief=tk.FLAT, padx=12, pady=10)
        ...
        ttk.Button(input_row, text="Send", command=self.send_message).pack(side=tk.LEFT, padx=(6, 0))
        ...
        ttk.Label(chat_tab, text="Tip: type 'open youtube', 'play spotify', 'launch notepad' etc. — Saathi acts on these instantly.", wraplength=1100).pack(anchor=tk.W, padx=10, pady=(0, 4))
        ttk.Label(chat_tab, textvariable=self.status_var).pack(anchor=tk.W, padx=10, pady=(0, 6))

        ttk.Label(actions_tab, text="Every action below needs your confirmation.", font=("Segoe UI", 12, "bold")).pack(anchor=tk.W, padx=15, pady=15)
        permissions = ttk.Frame(actions_tab)
        permissions.pack(fill=tk.X, padx=15, pady=(0, 8))
        ttk.Label(permissions, textvariable=self.permission_status).pack(side=tk.LEFT)
        ttk.Button(permissions, textvariable=self.permission_button, command=self.toggle_routine_permissions).pack(side=tk.RIGHT)
        action_grid = ttk.Frame(actions_tab)
        action_grid.pack(fill=tk.X, padx=15)
        for label, callback in [
            ...
            ttk.Button(action_grid, text=label, command=callback).pack(side=tk.LEFT, padx=5, pady=5)
        self.action_output = scrolledtext.ScrolledText(actions_tab, state="disabled", wrap=tk.WORD, height=14, bg="#08101e", fg="#d6efff", relief=tk.FLAT, padx=12, pady=10)
        self.action_output.pack(fill=tk.BOTH, expand=True, padx=15, pady=(15, 5))

        ttk.Label(actions_tab, text="Files, folders & commands — deletes and commands always ask for confirmation.", font=("Segoe UI", 10, "bold")).pack(anchor=tk.W, padx=15, pady=(4, 4))
        files_row = ttk.Frame(actions_tab)
        files_row.pack(fill=tk.X, padx=15, pady=(0, 4))
        self.path_input = ttk.Entry(files_row, width=45)
        ...
        ttk.Button(files_row, text="Delete", command=lambda: threading.Thread(target=self.delete_path, args=(self.path_input.get(),), daemon=True).start()).pack(side=tk.LEFT, padx=2)
        ...
        command_row = ttk.Frame(actions_tab)
        command_row.pack(fill=tk.X, padx=15, pady=(0, 12))
        self.command_input = ttk.Entry(command_row, width=45)
        self.command_input.pack(side=tk.LEFT, padx=(0, 6))
        ttk.Button(command_row, text="Run command", command=lambda: threading.Thread(target=self.run_command, args=(self.command_input.get(),), daemon=True).start()).pack(side=tk.LEFT, padx=2)
        ttk.Label(command_row, text="Also works in chat: 'run: dir', 'list files in Downloads', 'delete old.txt', 'weather in Kolkata', 'define serendipity'.", wraplength=550).pack(side=tk.LEFT, padx=10)

        ttk.Label(coding_tab, text="Create a safe starter project", font=("Segoe UI", 12, "bold")).pack(anchor=tk.W, padx=15, pady=(15, 5))
        form = ttk.Frame(coding_tab)
        ...
        self.project_name.grid(row=0, column=1, sticky=tk.W, pady=4)
        ttk.Label(form, text="Type:").grid(row=1, column=0, sticky=tk.W, pady=4)
        self.project_type = tk.StringVar(value="Python CLI")
        ttk.Combobox(form, textvariable=self.project_type, values=["Python CLI", "HTML website"], state="readonly", width=27).grid(row=1, column=1, sticky=tk.W, pady=4)
        ttk.Button(form, text="Choose projects folder", command=self.choose_workspace).grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=8)
        ttk.Button(form, text="Create starter project", command=self.create_project).grid(row=3, column=0, columnspan=2, sticky=tk.W)
        self.workspace_label = ttk.Label(coding_tab, text=f"Projects folder: {self.workspace}", wraplength=840)
        self.workspace_label.pack(anchor=tk.W, padx=15, pady=8)
        ttk.Label(coding_tab, text="Ask Saathi in Chat for features, then copy its code into your project. Running generated commands will be added only with a review screen.", wraplength=840).pack(anchor=tk.W, padx=15)
