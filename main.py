...
class Saathi:
    ...
    def process_user_text(self, text):
        ...
        if "independence day" in lowered and has_web_term:
            ...
            reply = (
                ...
                "• 3D Glowing Ashok Chakra (24 animated spokes)\n"
                "• 21st.dev Spotlight Mouse-Tracking Bento Grid\n"
                "• Web Audio API Patriotic Fanfare & Chimes\n"
                "• Physics Confetti Canon & National Salute Counter\n"
                "• Interactive Starlight Canvas Network\n\n"
                f"File generate karke aapke browser mein automatically open kar di hai: {file_path}"
            )
            self.messages.append(("assistant", reply))
            self.append_chat("assistant", reply)
            self.save_history()
        ...
        if (has_web_term and has_action_term) or any(p in lowered for p in ("website on ", "website for ", "webpage for ", "site on ", "site for ")):
            ...
            reply = (
                f"Bhai, '{topic}' ka GOD-LEVEL animated website 21st.dev spotlight cards aur extreme color skills ke saath ready hai! 🚀\n\n"
                ...
                "• Modular Glassmorphic Bento Grid\n\n"
                f"File save karke aapke default browser mein open kar di hai: {file_path}"
            )
            self.messages.append(("assistant", reply))
            self.append_chat("assistant", reply)
            self.save_history()
    ...
    def _create_independence_day_website(self):
        """Creates a God-Level Independence Day website in Projects/IndependenceDay/index.html and opens it."""
        target_dir = PROJECTS_DIR / "IndependenceDay"
        target_dir.mkdir(parents=True, exist_ok=True)
        html_file = target_dir / "index.html"
        html_content = web_engine.generate_independence_day_html()
        html_file.write_text(html_content, encoding="utf-8")
        webbrowser.open(html_file.as_uri())
        return html_file

    def create_god_level_website(self, topic="Modern Digital Experience"):
        """Generates a bespoke God-Level website with 21st.dev spotlight cards & extreme colors."""
    ...
    def demo_independence_day(self):
        ...
        reply = (
            ...
            "• Physics Confetti Canon & National Salute Counter\n"
            "• Interactive Starlight Canvas Network\n\n"
            f"File generate karke aapke browser mein open kar di hai: {file_path}"
        )
        self.messages.append(("assistant", reply))
        self.append_chat("assistant", reply)
        self.save_history()
